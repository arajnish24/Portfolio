...
import { ChevronLeft, Calendar, Clock, Eye, ThumbsUp, MessageSquare, Send, User } from 'lucide-react';
...
const BlogDetailsPage = () => {
  ...
  if (loading) {
    return <div className="text-center py-24 text-xs text-slate-500">Loading article details...</div>;
  }
  ...
  if (!blog) {
    return (
      <div className="container mx-auto px-6 py-24 max-w-xl text-center space-y-4">
        ...
        <Link to="/blogs" className="text-blue-400 font-bold hover:underline">Back to articles list</Link>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-6 py-12 max-w-4xl space-y-8 bg-grid-mesh">
      ...
      <div className="h-64 md:h-96 w-full rounded-3xl overflow-hidden border border-slate-800 shadow-2xl">
        <img
          src={blog.coverImage || 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=900'}
          alt={blog.title}
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-6 text-xs text-slate-500 font-semibold border-b border-slate-900 pb-6">
          <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5" /> {new Date(blog.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
          <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {blog.readingTime || '5 min read'}</span>
          <span className="flex items-center gap-1"><Eye className="h-3.5 w-3.5" /> {blog.views || 0} views</span>
          <button onClick={handleLike} className="flex items-center gap-1 text-slate-500 hover:text-rose-400 transition-colors">
            <ThumbsUp className="h-3.5 w-3.5 text-rose-500" /> {blog.likes || 0} likes
          </button>
        </div>
      </div>
      ...
      <article className="prose prose-invert max-w-none text-xs md:text-sm text-slate-300 leading-relaxed font-light space-y-6">
        {blog.content.split('\n').map((paragraph, index) => {
          if (paragraph.startsWith('# ')) {
            return <h2 key={index} className="text-xl md:text-2xl font-extrabold text-white mt-8 mb-4">{paragraph.replace('# ', '')}</h2>;
          }
          if (paragraph.startsWith('## ')) {
            return <h3 key={index} className="text-lg font-bold text-slate-200 mt-6 mb-3">{paragraph.replace('## ', '')}</h3>;
          }
          if (paragraph.startsWith('```')) {
      ...
      <div className="flex flex-wrap gap-2 border-t border-b border-slate-900 py-6">
        {blog.tags?.map((tag) => (
          <span key={tag} className="bg-slate-950 border border-slate-900/80 text-[10px] text-slate-400 font-bold px-3 py-1 rounded-lg">
            #{tag}
          </span>
        ))}
      </div>
      ...
      <div className="space-y-8 pt-4">
        ...
        <div className="space-y-4">
          {blog.comments?.map((comment) => (
            <div key={comment._id} className="glass-panel p-5 rounded-2xl border border-slate-900 flex gap-3">
              ...
              <div className="space-y-1.5 flex-1 text-xs">
                ...
                <p className="text-slate-400 leading-relaxed font-light">{comment.content}</p>
              </div>
            </div>
          ))}
          {(!blog.comments || blog.comments.length === 0) && (
            <p className="text-xs text-slate-600 italic">No comments posted yet. Start the conversation!</p>
          )}
        </div>

        {/* Add comment Form */}
        <form onSubmit={handleCommentSubmit} className="glass-panel p-6 rounded-3xl border border-slate-850 space-y-4">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Post a Comment</span>
