...
import { BookOpen, Search, Eye, ThumbsUp, Calendar, Clock, ChevronRight } from 'lucide-react';
    ...
    <div className="container mx-auto px-6 py-12 max-w-7xl space-y-10 bg-grid-mesh">
      ...
      <div className="text-center space-y-4 max-w-2xl mx-auto">
        ...
        <p className="text-sm text-slate-400 font-semibold leading-relaxed">
          Read my deep-dives on MERN development, cloud gateway architectures, identity verification tokens, and web application security pipelines.
        </p>
      </div>
      ...
      {loading ? (
        <div className="text-center text-xs text-slate-500 py-16">Loading articles...</div>
      ) : (
            ...
            >
              <div>
                <div className="h-48 w-full relative">
                  <img
                    src={blog.coverImage || 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600'}
                    alt={blog.title}
                ...
                <div className="p-6 space-y-3">
                  <div className="flex items-center gap-3 text-[10px] text-slate-500 font-semibold">
                    <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {new Date(blog.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                    <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {blog.readingTime || '5 min read'}</span>
                  </div>
              ...
              <div className="p-6 pt-0 border-t border-slate-900/60 mt-4 flex items-center justify-between">
                <div className="flex gap-4 text-[10px] text-slate-500 font-bold">
                  ...
                  <span className="flex items-center gap-1"><ThumbsUp className="h-3.5 w-3.5" /> {blog.likes || 0}</span>
                </div>
                <Link
                  to={`/blogs/${blog.slug}`}
