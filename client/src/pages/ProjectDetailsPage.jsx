...
const ProjectDetailsPage = () => {
  ...
  const fetchProjectDetails = () => {
    ...
    fetch(`/api/project/${id}`)
      ...
      .catch((err) => {
        console.warn(
          "Failed to fetch project details from server, using fallback details.",
        );
        ...
        const fallbackProject = {
          ...
          description:
            "A production-ready secure MERN portfolio system with dual-key token restrictions, resilient database failovers, and embedded recruiter analytics trackers. It prevents visual document spoofing and blocks spam publications.",
          technologies: [
            "React",
            "Express.js",
            "Node.js",
          ...
          {project.images && project.images.length > 1 && (
            <div className="flex gap-3 overflow-x-auto pb-2">
              {project.images.map((img, idx) => (
                <div
                  key={idx}
                  className="w-24 h-16 rounded-xl overflow-hidden border border-slate-900 shrink-0"
                >
                  <img
                    src={img}
                    alt="Screenshot"
            ...
            <div className="pt-2 border-t border-slate-900 flex flex-col gap-2.5 text-xs text-slate-400">
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-1.5">
                  <Hourglass className="h-4 w-4 text-purple-400" /> Duration
                </span>
              ...
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-1.5">
                  <Users className="h-4 w-4 text-emerald-400" /> Team Size
                </span>
                <strong className="text-white">
                  {project.teamSize || 1} Developer
                </strong>
              </div>
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-1.5">
                  <Layers className="h-4 w-4 text-amber-400" /> Difficulty
                </span>
                <strong className="text-white">
              ...
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-1.5">
                  <Cpu className="h-4 w-4 text-blue-400" /> Client
                </span>
                <strong className="text-white">
                  {project.clientName || "Personal"}
                </strong>
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2">
                {project.playStore && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="bg-slate-950 border border-slate-900 p-2 rounded-xl text-[10px] text-center text-slate-300 font-bold"
                  >
                    Play Store
                  </a>
                )}
          ...
          <div className="glass-panel p-6 rounded-3xl border border-slate-850 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {project.technologies?.map((tech) => (
                <span
                  key={tech}
                  className="bg-slate-950 border border-slate-900 px-3 py-1 rounded-xl text-xs text-slate-300 font-mono font-medium"
                >
                  {tech}
                </span>
              ))}
          ...
          <div className="glass-panel p-6 rounded-3xl border border-slate-850 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-900">
                ...
                <span className="text-xl font-extrabold mt-1 block text-rose-400">
                  {project.likes || 0}
                </span>
              </div>
            </div>
