...
const LoginPage = () => {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      setResetFlow(true);
      setForgotFlow(false);
      setStatusMsg({ type: 'success', text: 'Password reset OTP sent to email!' });
    } catch (err) {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setResetFlow(false);
      setDevOtp('');
      setStatusMsg({ type: 'success', text: 'Password reset successfully! Proceed to sign in.' });
    } catch (err) {
      setStatusMsg({ type: 'error', text: err.message });
    } finally {
      setLoading(false);
      ...
      <div className="glass-panel p-8 rounded-3xl border border-slate-800 shadow-2xl space-y-6 relative overflow-hidden">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-extrabold tracking-tight">
            {forgotFlow ? 'Request Reset' : resetFlow ? 'Reset Password' : 'Welcome Back'}
          </h2>
          <p className="text-xs text-slate-400 font-semibold">
            {forgotFlow ? 'Enter your registered email below' : resetFlow ? 'Enter the OTP and new password' : 'Log in to your secure admin workspace'}
          </p>
        </div>

        {statusMsg.text && (
          <div className={`p-4 rounded-xl text-xs font-semibold ${statusMsg.type === 'success' ? 'bg-emerald-950/40 text-emerald-400 border border-emerald-900' : 'bg-rose-950/40 text-rose-300 border border-rose-900'}`}>
            {statusMsg.text}
          </div>
        )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  type="email"
                  value={email}
                  onChange={(e) => { setEmail(e.target.value); setStatusMsg({ type: '', text: '' }); }}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  value={password}
                  onChange={(e) => { setPassword(e.target.value); setStatusMsg({ type: '', text: '' }); }}
        ...
        {resetFlow && (
          <form onSubmit={handleResetPassword} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs text-slate-400 font-bold uppercase tracking-wider block">6-Digit Reset OTP</label>
              <input
                type="text"
                maxLength={6}
                value={otpCode}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-slate-400 font-bold uppercase tracking-wider block">New Password</label>
              <input
                type="password"
                value={newPassword}
