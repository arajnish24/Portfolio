...
export const seedMongoDb = async () => {
  try {
    ...
    if (!owner) {
      ...
      const hashedPassword = plainPassword ? await bcrypt.hash(plainPassword, salt) : "";
      ...
      owner = new User({
        ...
        role: "Owner",
        isVerified: true,
        portfolioToken: process.env.OWNER_PORTFOLIO_TOKEN,
      ...
      const defaultBlogs = [
        {
          ...
          category: "Security",
          content:
            "Security is paramount in web development. In this article, we cover double-key authentication, OTP logic, and headers injection mitigations...",
