  ...
  async (req, res) => {
    ...
    if (!password) {
      return res
        .status(400)
        .json({
          message: "Password verification required to regenerate token",
        });
    }
    ...
    try {
      ...
      if (!isMatch) {
        return res
          .status(400)
          .json({ message: "Invalid password. Verification failed." });
      }
...
router.post("/verify-project-token", requireAuth, requireOwner, (req, res) => {
  ...
  if (!token) {
    return res
      .status(400)
      .json({ message: "Portfolio Verification Token is required" });
  }
  if (!emailOtp) {
    return res
      .status(400)
      .json({ message: "Email Verification OTP is required" });
  }

  if (token !== req.user.portfolioToken) {
    return res.status(401).json({
      message:
        "🔴 TOKEN VERIFICATION REJECTED: Invalid Portfolio Token. You are not authorized to edit projects.",
    });
  }
  ...
  if (!isOtpValid) {
    return res.status(401).json({
      message:
        "🔴 OTP VERIFICATION REJECTED: Invalid or expired email OTP. Please try again.",
    });
  }
