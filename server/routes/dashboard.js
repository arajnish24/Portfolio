...
router.get('/analytics', requireAuth, requireOwner, async (req, res) => {
  try {
    ...
    // Calculations
    const profileViews = logs.filter(l => l.actionType === 'profile_view').length;
    const resumeDownloads = logs.filter(l => l.actionType === 'resume_download').length;
    const projectClicks = logs.filter(l => l.actionType === 'project_click').length;
...
router.put('/notifications/read', requireAuth, requireOwner, async (req, res) => {
  try {
    const isMock = checkDbMode();
