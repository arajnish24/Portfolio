...
const verifyTokenHeader = (req, res, next) => {
  ...
  if (!portfolioToken) {
    return res
      .status(401)
      .json({
        message: "Portfolio token is required for this write operation",
      });
  }
  ...
  if (portfolioToken !== req.user.portfolioToken) {
    return res.status(403).json({
      message:
        "🔴 TRANSACTION ABORTED: The verification token entered is invalid.",
    });
  }
  next();
};
  ...
  try {
    ...
    if (!isOwnerRequest) {
      projects = projects.filter(
        (p) => !p.isHidden && p.status === "published",
      );
    }
