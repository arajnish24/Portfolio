...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    cb(
      new Error(
        "Supported formats: jpg, jpeg, png, gif, webp, svg, pdf, doc, docx",
      ),
    );
  },
});
...
export const logAnalytics = async (req, actionType, details = {}) => {
  try {
    ...
    const browser = req.headers["user-agent"]?.includes("Firefox")
      ? "Firefox"
      : "Chrome";
    const os = req.headers["user-agent"]?.includes("Macintosh")
      ? "MacOS"
      : "Windows";
  ...
  try {
    ...
    if (useMock) {
      ...
      if (owner) {
        ...
        const filterOwnerItems = (col) =>
          mockDbHelper
            .getCollection(col)
            .filter((x) => x.ownerId === owner._id);
        ...
        blogs = filterOwnerItems("blogs").filter(
...
router.put("/appearance", requireAuth, requireOwner, async (req, res) => {
  const { theme, accentColor, fontFamily, layout, animationsEnabled } =
    req.body;
